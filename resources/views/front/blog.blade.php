        ...
        <div class="col-md-6 col-lg-4 wow fadeInUp" data-wow-delay="0.1s">
            <div class="blog-grid">
                ...
                <div class="blog-content">
                    <div class="blog-meta style2"><a href="{{ route('blog_details' , [ 'slug'=> $blog->translate(app()->getLocale())->slug ]) }}">{{ $blog->translate(app()->getLocale())->name }} </a> <a
                            href="blog-details.html"> {{ $blog->created_at }} </a></div>
                    <p class="blog-text">{!! substr(strip_tags($blog->des), 0, 150) !!}</p><a
                        href="{{ route('blog_details' , [ 'slug'=> $blog->translate(app()->getLocale())->slug ]) }}" class="link-btn">اقرأ المزيد</a>
                </div>
            </div>
        </div>
        @endforeach
