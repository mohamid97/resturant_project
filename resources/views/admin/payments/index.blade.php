@extends('admin.layout.master')
@section('styles')
<style>
 .pay_header{
    width: 100%;
    display: flex;
    justify-content: space-between;
 }
 .pay_header img{
    border: 3px solid #f1f1f1;

 }
</style>
@endsection
@section('content')
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1>@lang('sidebar.payments')</h1>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="#">@lang('main.home')</a></li>
                        <li class="breadcrumb-item active">@lang('sidebar.payments') </li>
                    </ol>
                </div>
            </div>
        </div><!-- /.container-fluid -->
    </section>

    <section class="content">
        <div class="container-fluid">

       
          <div class="row">
            <div class="col-md-4">
                <div class="card card-info">
                    <div class="card-header">
                        <h3 class="card-title pay_header">@lang('sidebar.paymob') 
                            <img height="40px" width="60px" src="data:image/png;base64,">
                        </h3>
                    </div>
                    <form method="post" action="{{ route('admin.payments.settings') }}">
                        @csrf
                        <input type="hidden" name="payment_method" value="paymob" />
                        <div class="card-body">
                            <div class="form-group">
                                <label for="paymob_api">Paymob Api Key</label>
                                <input type="text" class="form-control"  name="paymob_api" id="paymob_api" placeholder="Paymob Api Key" value="{{ isset($paymob)? $paymob->paymob_api : '' }}">
                                @error('paymob_api')
                                <div class="text-danger">{{ $errors->first('paymob_api') }}</div>
                                @enderror
                            </div>


                            <div class="form-group">
                                <label for="paymob_card_id">Paymob Card id </label>
                                <input type="text" class="form-control" name="paymob_card_id" id="paymob_card_id" placeholder="Paymob Card Id" value="{{ isset($paymob)? $paymob->paymob_card_id : '' }}">
                                @error('paymob_card_id')
                                <div class="text-danger">{{ $errors->first('paymob_card_id') }}</div>
                                @enderror
                            </div>


                            <div class="form-group">
                                <label for="paymob_card_iframe">Paymob Card Iframe </label>
                                <input type="text" class="form-control" name="paymob_card_iframe" id="paymob_card_iframe" placeholder="Paymob Card iframe" value="{{ isset($paymob)? $paymob->paymob_card_iframe : '' }}">
                                @error('paymob_card_iframe')
                                <div class="text-danger">{{ $errors->first('paymob_card_iframe') }}</div>
                                @enderror
                            </div>


                        


                            <div class="form-group">
                                <label for="paymob_wallet_id">Wallet Id </label>
                                <input type="text" class="form-control" id="paymob_wallet_id" name="paymob_wallet_id" placeholder="Paymob Wallet Id" value="{{ isset($paymob)? $paymob->paymob_wallet_id : '' }}">
                                @error('paymob_wallet_id')
                                <div class="text-danger">{{ $errors->first('paymob_wallet_id') }}</div>
                                @enderror
                            </div>




    
                        </div>

                        <div class="card-footer">
                            <button type="submit" class="btn btn-info"> <i class="nav-icon fas fa-paper-plane"></i> @lang('main.submit')</button>
                           
                        </div>
    
                    </form>
                </div>
            </div>


            <div class="col-md-4">
                <div class="card card-info">
                    <div class="card-header">
                        <h3 class="card-title pay_header">@lang('sidebar.paypal') 
                            <img height="40px" width="60px" src="https://tunisie-telegraph.com/wp-content/uploads/2022/05/paypal.jpg">
                        </h3>
                    </div>
                    <form method="post" action="{{ route('admin.payments.settings') }}">
                        @csrf
                        <input type="hidden" name="payment_method" value="paypal" />
                        <div class="card-body">
                            <div class="form-group">
                                <label for="paypal_client_id">Paypal Client Id</label>
                                <input type="text" class="form-control"  name="paypal_client_id" id="paypal_client_id" placeholder="PayPal Client Id" value="{{ isset($paypal)? $paypal->paypal_client_id : '' }}">
                                @error('paypal_client_id')
                                <div class="text-danger">{{ $errors->first('paypal_client_id') }}</div>
                                @enderror
                            </div>


                            <div class="form-group">
                                <label for="paypal_client_secret">Paypal Client Secret </label>
                                <input type="text" class="form-control" name="paypal_client_secret" id="paypal_client_secret" placeholder="Paymob Client Secret" value="{{ isset($paypal)? $paypal->paypal_client_secret : '' }}">
                                @error('paypal_client_secret')
                                <div class="text-danger">{{ $errors->first('paypal_client_secret') }}</div>
                                @enderror
                            </div>




                        






    
                        </div>

                        <div class="card-footer">
                            <button type="submit" class="btn btn-info"> <i class="nav-icon fas fa-paper-plane"></i> @lang('main.submit')</button>
                           
                        </div>
    
                    </form>
                </div>
            </div>




            <div class="col-md-4">
                <div class="card card-info">
                    <div class="card-header">
                        <h3 class="card-title pay_header">@lang('sidebar.stripe')

                            <img height="40px" width="60px" src="data:image/png;base64,">
                        </h3>
                    </div>
                    <form method="post" action="{{ route('admin.payments.settings') }}">
                        @csrf
                        <input type="hidden" name="payment_method" value="stripe" />
                        <div class="card-body">
                            <div class="form-group">
                                <label for="stripe_key">Stripe Key</label>
                                <input type="text" class="form-control"  name="stripe_key" id="stripe_key" placeholder="Stripe Key" value="{{ isset($stripe)? $stripe->stripe_key : '' }}">
                                @error('stripe_key')
                                <div class="text-danger">{{ $errors->first('stripe_key') }}</div>
                                @enderror
                            </div>


                            <div class="form-group">
                                <label for="stripe_secret">Stripe Secret  </label>
                                <input type="text" class="form-control" name="stripe_secret" id="paypal_client_secret" placeholder="Stripe Secret" value="{{ isset($stripe)? $stripe->stripe_secret : '' }}">
                                @error('stripe_secret')
                                <div class="text-danger">{{ $errors->first('stripe_secret') }}</div>
                                @enderror
                            </div>




                        






    
                        </div>

                        <div class="card-footer">
                            <button type="submit" class="btn btn-info"> <i class="nav-icon fas fa-paper-plane"></i> @lang('main.submit')</button>
                           
                        </div>
    
                    </form>
                </div>
            </div>



          </div>




        </div>
    </section>
@endsection
